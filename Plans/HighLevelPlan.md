...
6. **New device queue and badge**
   ...
   - macOS usually switches to a newly connected device on its own. Slice 4 adopts that as an override when the device has never been seen, so the new device does play, the badge says it needs sorting, and nothing is lost if it is never sorted. A queued device reconnecting counts as new too, so it plays each time it comes back until it is placed (see Decisions).
   - The badge is drawn into the menu bar image, the same approach as locus-launcher's `MenuBarIcon`
   - Screen sharing and AirPlay taps get a new UID per session (see Decisions). They never enter the queue and never badge, or the icon would light up after every screen share. Screen sharing is recognized by its `-screen` suffix. The AirPlay tap's UID shape has not been seen yet, so it is not filtered; add it once one turns up in the device history.
   ...
   - The match (`ModelMatch`) leaves Bluetooth out. A Bluetooth UID is the device's own address and never drifts, so a new one is a different unit, and matching it by model would fold a second pair of the same AirPods into the first pair's entry.
   ...
   - Dragging is not the only way out of the queue. "Add to Priority Order" in the menus and on the row puts a device at the bottom of the order, for anyone not using a pointer.
   - The menu's "Sound Devices…" item carries a count of queued devices, so the badge on the menu bar icon says where it is dealt with.
...
7. **iCloud sync**
   ...
   - What syncs about a device is what was decided about it: its UIDs, hidden, queued and assigned icon. Its name and guessed icon are read by each Mac from the device and are kept local, because they can differ between Macs. Both Macs report `BuiltInSpeakerDevice`, but one may call it "MacBook Pro Speakers" and the other "iMac Speakers"; syncing the name would have each Mac write its own back every time its device list settled, forever. A device this Mac has never had connected shows the name and icon the other Mac sent.
   - A new device's entry id is derived from its UID, so two Macs that meet the same device before either has synced it make one entry between them. Without that, a new pair of AirPods connecting to both Macs within the few seconds iCloud takes would be listed twice on both. Entries made before this keep their random ids.
...
- **The order is last-writer-wins; per-device flags merge.** An ordered list cannot be merged sensibly. A drag touches many positions at once, so per-position keys would still produce interleavings nobody chose, which is worse than losing a reorder. The whole order is one value, and reordering is rare and deliberate enough that redoing it costs seconds. The per-device flags — hidden, queued, forgotten, assigned icon — are independent, so each device gets its own key and edits on two Macs to different devices merge cleanly. This is the same split locus-launcher uses for hot keys, and the same merge: each side is compared with what the two last agreed on, to tell which one changed.

  No timestamps were needed. iCloud key-value storage already keeps the last write to each key, so two Macs changing the same thing while both are syncing resolve there. The merge only sees a conflict when this Mac changed something while it was not syncing, and then iCloud's version wins, since the other Macs already have it. Slice 6's merge keeps the highest entry's icon rather than the most recently assigned one, since nothing records when an icon was assigned.
...
- **A queued device plays whenever macOS switches to it.** A queued device is never chosen automatically, so macOS switching to it as it connects is the only way it plays. That switch is adopted as an override each time it returns, not only the first time, so a device behaves the same until the user places it. The cost is that a queued pair of headphones reconnecting replaces an override, which is the complaint this app exists for; the badge is what asks for it to be placed, and placing it ends that.

- **The Sound Devices window's explanation panels are rows, not section headers.** A `List` gives a section header a fixed height, so an explanation that wraps is either cut to one line or clipped. As rows they size to their text, and they are not selectable because they carry no tag.
